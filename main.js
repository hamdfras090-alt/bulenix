...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- Theme Toggle Logic (Night/Morning Mode) ---
    ...
    if (logoLink) {
        logoLink.addEventListener('click', (e) => {
            ...
            // Trigger professional loader during theme switch
    ...
    function renderProducts(filterCategory = 'all', shouldScroll = false) {
        ...
        if (productSectionTitle) {
            if (filterCategory === 'all') {
                productSectionTitle.innerHTML = 'اكتشف أفضل ما لدينا لشتاء 2026';
            } else {
                ...
                productGrid.innerHTML = filteredProducts.map(p => `
                    <div class="product-card" data-product-id="${p.id}" onclick="showProductDetails('${p.id}')">
                        ...
                        <div class="product-info">
                            ...
                            <button class="primary-btn full-width" style="margin-top:10px;" onclick="event.stopPropagation(); addToCart('${escapeHtml(p.title)}', '${p.price}', '${escapeHtml(p.img)}'); triggerSuccessFeedback(this)">أضف للسلة</button>
                        </div>
                    </div>
                `).join('');
            }

            // Click events are handled via inline onclick for better reliability in overlays
            lucide.createIcons();
        }, (error) => {
    ...
    window.openCategoryPage = (categoryName) => {
        ...
        triggerLoader(() => {
            ...
            if (filtered.length === 0) {
                ...
                // Click events handled via inline onclick
            }
    ...
    function renderNewProductsSlider() {
        ...
        slider.innerHTML = latestProducts.map(p => {
            ...
            return `
        <div class="slider-product-card" data-product-id="${p.id}" onclick="showProductDetails('${p.id}')">
            ...
            <div class="slider-product-info">
                <h3 class="product-title">${p.title}</h3>
                <div class="slider-product-price">${price.toFixed(2)} د.ل</div>
                <button class="slider-add-btn" onclick="event.stopPropagation(); addToCart('${escapeHtml(p.title)}', '${p.price}', '${escapeHtml(p.img)}'); triggerSuccessFeedback(this)">أضف للسلة</button>
            </div>
        </div>
    `}).join('');
